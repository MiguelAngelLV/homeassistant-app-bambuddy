...
Almost everyone is upgrading from 0.2.4. 0.2.4.1 is a patch release: stability and correctness fixes built on the same code base as 0.2.4, no schema breaks, no Docker entrypoint changes, no Vite/proxy quirks.

⚠ **In-app Apply Update does NOT work for native installs upgrading from 0.2.4 to 0.2.4.1.** A separate fix in this release (`fix(updates): add --force to git fetch`) is itself what 0.2.4's updater needs to succeed. Clicking *Settings → System → Updates → Apply Update* on a 0.2.4 native install will fail with "Failed to fetch updates" — the underlying error is `! [rejected] v0.2.1 -> v0.2.1 (would clobber existing tag)`, because an old tag on the remote was re-pointed and 0.2.4's updater can't overwrite it locally. Use one of the two CLI paths below instead. Once you're on 0.2.4.1, the in-app updater works again for every future release. **Docker users are unaffected** — they don't go through git at all.
...
`update.sh` works because it fetches without `--tags`, so it never hits the tag-clobber issue. Snapshots the database first and rolls back on failure.

**Native install — manual path** (only if `update.sh` is unavailable)
...
The explicit `--force` on the fetch is what the in-app updater is missing on 0.2.4 — it lets your local clone overwrite the stale `v0.2.1` tag with the remote's current target. `git reset --hard origin/main` is required because a plain `git checkout main` won't fast-forward if your local main has drifted.
